...
for (let e of x) {
  let i = obj.find((f) => {
    if (
      e.name.startsWith("Calgary - Nose Hill") &&
      f.name == "Calgary - Nosehill"
    ) {
      return true;
    }

    if (
      e.name.startsWith("Cochrane") &&
      f.name.startsWith("Cochrane") &&
      e != f
    ) {
      return true;
    }
    if (e.name.search(f.name) == 0 || f.name.search(e.name) == 0) {
      if (
        (e.name.length > f.name.length && e.name[f.name.length + 1] == "(") ||
        (f.name.length > e.name.length && f.name[e.name.length + 1] == "(")
      )
        return true;
      else return false;
    } else {
